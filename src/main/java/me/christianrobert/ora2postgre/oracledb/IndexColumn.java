...
public class IndexColumn {
    ...
    /**
     * Gets the PostgreSQL-compatible column reference with proper ordering.
     */
    public String getPostgreSQLColumnReference() {
        String reference = getEffectiveColumnReference();
        
        // Apply sort order if specified
        if (sortOrder != null && !sortOrder.trim().isEmpty()) {
            if ("DESC".equalsIgnoreCase(sortOrder.trim())) {
                reference += " DESC";
            } else if ("ASC".equalsIgnoreCase(sortOrder.trim())) {
                reference += " ASC";
            }
        } else if (descending) {
            reference += " DESC";
